...
st.write(f"Data de modificação selecionada: {data_modificada_formatada}")

# Coleta e processa os dados dos formulários
com_box_disponibilidade = st.checkbox("Disponibilidade")
...
if st.button("Salvar Dados"):
    if professor and unidades:
        nova_linha = {
            'Professor': professor,
            'Unidades': unidades,
            'Carro': carro,
            'Máquinas': maquinas,
            'Disponibilidade': com_box_disponibilidade,
            'Módulo': modulo,
            'Observações': observacoes,
            'Nome do Preenchendor': nome_preenchedor,
            'Data': data_modificada_formatada
        }
        st.session_state.df_disponibilidade = st.session_state.df_disponibilidade.append(nova_linha, ignore_index=True)
        salvar_dados(st.session_state.df_disponibilidade)
...
# Interface para deletar uma linha
if st.button("Deletar Linha"):
    index = st.number_input("Número do Índice para Deletar", min_value=0, max_value=len(st.session_state.df_disponibilidade)-1)
    deletar_linha(index)
