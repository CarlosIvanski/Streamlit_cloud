...
# Nome inicial do professor
nome_inicial = 'Pessoa A'
...
if 'disponibilidade' not in st.session_state:
    st.session_state.disponibilidade = {nome_inicial: {}}
...
# Mostrando apenas um bloco para o professor
cols = st.columns(col_widths)

with cols[0]:
    nome_professor = st.text_input(f"Nome do professor:", nome_inicial, key=f"nome_0")
...
# Opções para deletar uma linha
st.subheader("Deletar linha")
if not st.session_state.df_disponibilidade.empty:
    indices = st.selectbox("Selecione o índice para deletar", st.session_state.df_disponibilidade.index)
    if st.button("Deletar"):
        deletar_linha(indices)
